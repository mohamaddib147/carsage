...
- `frontend/src/pages/CarProfilePage.jsx` — Car Profile screen (basic feature): identity card (icon, Make/Model, Year · Fuel Type) plus a Vehicle Specifications card listing all `cars` fields (including Cylinders/Drivetrain/Transmission from CAR-34) as icon rows, for the logged-in user's car (via `/cars/mine`, or a specific `/cars/:carId`); Edit mode (2-column field grid) updates any field via `cars` update; empty state (no car yet) links to Car Onboarding. Relies on RLS to scope view/edit to the owner. Fuel Efficiency is labeled km/L (not mpg — fixed in CAR-16). Layout matches `docs/stitch_carsage_landing_page/carsage_car_profile`'s card/icon-row style; its telemetry badges (health score, keyfob, vault ID, NHTSA-validated tag) are out of scope, omitted.
- `frontend/src/pages/CarProfilePage.test.jsx` — tests viewing all fields (including the CAR-34 spec fields), a placeholder dash for unset spec-autofill fields, the empty state, editing/saving a field (correct `id` scoping), edit validation, update failure, Cancel discarding changes, special characters in VIN rendering as safe text (no XSS), and that a car id the user doesn't own renders the same safe not-found state.
- `frontend/src/pages/TripPlannerPage.jsx` — Trip Planner screen (core feature): a Route Parameters card with Starting Location (optional) + Destination (required) fields, calls the FastAPI cost-estimate endpoint with a loading state, shows a 3-stat results card (Fuel Cost, Duration, Distance) with a route heading and captions, or a clear error. Uses the logged-in user's own car automatically (no selector — MVP is one car). Empty state if the user has no car yet. Layout matches `docs/stitch_carsage_landing_page/carsage_trip_planner`'s card style; its car-switcher chip, weather widget, and route-recommendation badges are out of scope, omitted.
- `frontend/src/pages/TripPlannerPage.test.jsx` — tests the no-car empty state, required-destination validation, the loading state + results card on success, submitting with no Starting Location, and the clear-error case.
