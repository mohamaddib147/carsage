...
- `frontend/src/pages/TripPlannerPage.jsx` — Trip Planner screen (core feature): Starting Location (optional) + Destination (required) fields, calls the FastAPI cost-estimate endpoint with a loading state, shows a results card (Fuel Cost, Duration, Distance) or a clear error. Uses the logged-in user's own car automatically (no selector — MVP is one car). Empty state if the user has no car yet.
