...
- `frontend/src/pages/CarProfilePage.jsx` — Car Profile screen (basic feature): identity card (icon, Make/Model, Year · Fuel Type) plus a Vehicle Specifications card listing all `cars` fields as icon rows, for the logged-in user's car (via `/cars/mine`, or a specific `/cars/:carId`); Edit mode (2-column field grid) updates any field via `cars` update; empty state (no car yet) links to Car Onboarding. Relies on RLS to scope view/edit to the owner. Fuel Efficiency is labeled km/L (not mpg — fixed in CAR-16). Layout matches `docs/stitch_carsage_landing_page/carsage_car_profile`'s card/icon-row style; its telemetry badges (health score, keyfob, vault ID, NHTSA-validated tag) are out of scope, omitted.
