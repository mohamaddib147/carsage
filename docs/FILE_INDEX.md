...
- `backend/app/services/vehicle_lookup.py` — CAR-34 spec autofill: `lookup_nhtsa` confirms a make/model/year exists via NHTSA vPIC and returns an approximate vehicle type as `engine_type` (vPIC has no free per-model engine spec without a real VIN); `lookup_api_ninjas` queries API Ninjas' Cars API for fuel economy (converted from combined MPG to km/L), cylinders, drivetrain, and transmission, returning all-null if `API_NINJAS_KEY` isn't set or there's no match; `get_spec_suggestions` combines both. Every function is best-effort and never raises.
...
- `backend/tests/test_vehicle_lookup.py` — tests `lookup_nhtsa` (confirmed model + resolved vehicle type, unrecognized model, network failure), `lookup_api_ninjas` (MPG→km/L conversion, no configured key, no match, network failure — all never raise), and `get_spec_suggestions`'s merge of both.
